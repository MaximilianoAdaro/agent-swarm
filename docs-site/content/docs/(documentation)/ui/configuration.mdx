...
When an agent goes over, its least-recently-used **live** sessions are closed first, so the session it
is actively using survives and abandoned ones are shed. If in-flight admissions alone fill the cap
there is nothing left to shed, and the new `initialize` is refused with **HTTP 429** and a JSON-RPC
error telling the client to close a session and retry. A normal worker uses a single session; raise
this only for an agent that legitimately runs several MCP clients in parallel.
...
a worker that is idle mid-task has to re-initialize its session. The value is re-read on each sweep,
so a change applies on the next tick without a restart.
