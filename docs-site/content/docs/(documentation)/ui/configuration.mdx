...
`MCP_MAX_SESSIONS_PER_AGENT` (default `16`) caps how many live sessions one agent may hold. When an
agent opens one past the cap, its least-recently-used sessions are closed first, so the session it
is actively using survives and abandoned ones are shed. A normal worker uses a single session; raise
