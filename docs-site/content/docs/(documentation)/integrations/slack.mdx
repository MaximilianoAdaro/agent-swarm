...
- Files shared in assistant threads — with or without a caption — reach the task as attachments (see [Attachment Handling](#attachment-handling))
...
Each file also appears in the task description as a metadata line (filename, MIME type, size, Slack file ID). A file that can't be attached — larger than the 50 MB attachment limit, a failed download, or a storage error — is marked `(not attached: <reason>)` on that line, and the bot replies in the thread naming the file and the reason.

Limitations:
- With `ADDITIVE_SLACK=true`, follow-ups that are buffered (rather than turned into a task right away) carry only the metadata line; the file is not attached.
